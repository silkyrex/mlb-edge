...
Usage:
  python scout.py teams [--hot-only]
  python scout.py pitchers [--starters-only] [--hot-only]
...
def teams(hot_only: bool = False):
    ...
    season = conn.execute("""
        SELECT t.name,
               ...
               ROUND(AVG(t.ra), 2) as rag,
               ROUND(AVG(t.rf - t.ra), 2) as diff
        FROM (
    ...
    recent = conn.execute("""
        SELECT t.name, ROUND(AVG(t.rf - t.ra), 2) as diff_recent
        FROM (
            SELECT home_team as name, home_score as rf, away_score as ra FROM games
            WHERE home_score IS NOT NULL AND date >= date('now', '-14 days')
            UNION ALL
            SELECT away_team, away_score, home_score FROM games
    ...
    print(f"\nTEAM TIERS  (run differential -- RS/G minus RA/G)  --  ELITE = bet on | FADE = bet against\n")
    print(f"{'Tier':<6} {'Team':<28} {'W-L':>5} {'RS/G':>5} {'RA/G':>5} {'DIFF':>5} {'Trend':>5}")
    print("-" * 68)
    ...
    for i, r in enumerate(season):
        ...
        print(f"{t:<6} {r['name']:<28} {wl:>5} {r['rsg']:>5} {r['rag']:>5} {diff_str:>5} {tr:>5}")
...
def pitchers(starters_only: bool = False, hot_only: bool = False):
    ...
    print(f"\n{label} TIERS  (season ERA, min {min_ip} IP)  --  ELITE = low ERA | FADE = high ERA\n")
...
def hitters(hot_only: bool = False):
    ...
    flag = "  (hot-only)" if hot_only else ""
    print(f"\nHITTER TIERS  (season AVG + K%){flag}  --  ELITE = bet on | FADE = bet against\n")
    print(f"{'Tier':<6} {'Name':<25} {'Team':<25} {'AB':>4} {'AVG':>5} {'AVG 14d':>7} {'K%':>6} {'HR':>4} {'Trend':>5}")
    print("-" * 96)
    ...
    for i, r in enumerate(season):
        ...
        krate = f"{r['krate']:.3f}" if r["krate"] is not None else "-"
        print(f"{t:<6} {r['name']:<25} {r['team']:<25} {r['ab']:>4} {r['avg']:>5} {avg14:>7} {krate:>6} {r['hr']:>4} {tr:>5}")
